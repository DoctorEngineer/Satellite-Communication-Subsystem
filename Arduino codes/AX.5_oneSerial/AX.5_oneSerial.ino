 ...
 * RETURN : void
 ******************************************************************************/
void ssp_protocol(int length_SSP_recieveFrame)
{
    SSP_recieveFrame_length = length_SSP_recieveFrame;
...
void SSP_frame_processing(SSP_buff_gen_t* buff)
{
  ...
  { 
    //check crc
    //check if data exist- process TYPE and data- IF there is data
    if (SSP_recieveFrame_length > 7) 
    {
      switch (SSP_recieve_buffer[3])
     {
      case SET_OBC_TIME:
          ...
          SSP_data_length = (SSP_data_length + 4) - flag1;
          SSP_Buffer_Gen(&SSP_frame_content);
          SSP_transmit_func();  // OBC Connected to Serial2
